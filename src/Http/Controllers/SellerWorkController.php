...
final class SellerWorkController{
 ...
 public function reportDamage(Request $r,array $p):void{
  ...
  try{
   ...
   $run=$this->db->prepare('SELECT id FROM order_runs WHERE order_id=? ORDER BY run_no DESC LIMIT 1');$run->execute([$o['id']]);$runId=(int)$run->fetchColumn();
   $this->db->beginTransaction();
   $captureSource=in_array((string)$r->input('capture_source'),['live_camera','file_picker'],true)?(string)$r->input('capture_source'):'file_picker';$meta=json_encode(['capture_source'=>$captureSource,'user_agent'=>$r->server['HTTP_USER_AGENT']??null],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$this->db->prepare("INSERT INTO evidences(order_id,order_run_id,order_component_id,evidence_type,file_path,original_name,mime_type,file_size,sha256,captured_at,metadata_json,review_status,created_at) VALUES(?,?,?,'damage_initial',?,?,?,?,?,NOW(),?,'pending',NOW())")->execute([$o['id'],$runId,$componentId,$file['path'],$file['original_name'],$file['mime'],$file['size'],$file['sha256'],$meta]);
