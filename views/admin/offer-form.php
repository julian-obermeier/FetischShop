...
<label class="span-2">Aufnahme<select name="evidence_camera_required[]"><option value="1" <?=(!array_key_exists('camera_required',$w)||!empty($w['camera_required']))?'selected':''?>>Nur Direktkamera</option><option value="0" <?=array_key_exists('camera_required',$w)&&empty($w['camera_required'])?'selected':''?>>Dateiauswahl erlaubt</option></select></label>
<div class="span-1 remove-row"><button type="button" class="btn ghost" data-remove-row>Entfernen</button></div>
</div>
<?php endforeach;?>
</div>
</section>
...
<div class="structured-row" data-repeat-row>
<label class="span-3">Bezeichnung<input name="precheck_label[]" value="<?=App\Core\View::e($pr['label']??'')?>"></label>
<label class="span-4">Beschreibung<input name="precheck_description[]" value="<?=App\Core\View::e($pr['description']??'')?>"></label>
<label class="span-1">Anzahl<input name="precheck_count[]" type="number" min="1" value="<?=max(1,(int)($pr['required_count']??1))?>"></label>
<label class="span-2">Aufnahme<select name="precheck_camera_required[]"><option value="1" <?=(!array_key_exists('camera_required',$pr)||!empty($pr['camera_required']))?'selected':''?>>Nur Direktkamera</option><option value="0" <?=array_key_exists('camera_required',$pr)&&empty($pr['camera_required'])?'selected':''?>>Dateiauswahl erlaubt</option></select></label>
...
<template id="tpl-evidence-window"><div class="structured-row" data-repeat-row><label class="span-3">Bezeichnung<input name="evidence_name[]" placeholder="z. B. Abend"></label><label class="span-2">Beginn<input name="evidence_start[]" type="time"></label><label class="span-2">Ende<input name="evidence_end[]" type="time"></label><label class="span-2">Anzahl Fotos<input name="evidence_count[]" type="number" min="1" value="1"></label><label class="span-2">Aufnahme<select name="evidence_camera_required[]"><option value="1" selected>Nur Direktkamera</option><option value="0">Dateiauswahl erlaubt</option></select></label><div class="span-1 remove-row"><button type="button" class="btn ghost" data-remove-row>Entfernen</button></div></div></template>
<template id="tpl-precheck"><div class="structured-row" data-repeat-row><label class="span-3">Bezeichnung<input name="precheck_label[]" placeholder="z. B. Artikel Vorderseite"></label><label class="span-4">Beschreibung<input name="precheck_description[]" placeholder="Was muss sichtbar sein?"></label><label class="span-1">Anzahl<input name="precheck_count[]" type="number" min="1" value="1"></label><label class="span-2">Aufnahme<select name="precheck_camera_required[]"><option value="1" selected>Nur Direktkamera</option><option value="0">Dateiauswahl erlaubt</option></select></label><div class="span-2 remove-row"><button type="button" class="btn ghost" data-remove-row>Entfernen</button></div></div></template>
