...
final class AdminController{
 ...
 public function offerCreateForm():void{$cats=$this->db->query("SELECT id,name FROM categories WHERE is_active=1 ORDER BY sort_order,name")->fetchAll();View::render($this->root,'admin/offer-form',['pageTitle'=>'Angebot anlegen','offer'=>null,'version'=>null,'categories'=>$cats]);}
 public function createOffer(Request $r):void{
  $this->db->beginTransaction();try{
   ...
   $q=$this->db->prepare("INSERT INTO offers(category_id,seller_id,title,status,is_private,acceptance_deadline,created_at,updated_at) VALUES(?,?,?,?,?,?,NOW(),NOW())");$q->execute([$category,$seller,$title,(string)$r->input('status','draft'),$private,$r->input('acceptance_deadline')?:null]);$offerId=(int)$this->db->lastInsertId();
 ...
 public function offerEditForm(Request $r,array $p):void{$id=(int)$p['id'];$q=$this->db->prepare('SELECT * FROM offers WHERE id=?');$q->execute([$id]);$o=$q->fetch();if(!$o)Response::abort(404);$v=$this->db->prepare('SELECT * FROM offer_versions WHERE id=?');$v->execute([$o['current_version_id']]);$cats=$this->db->query("SELECT id,name FROM categories ORDER BY sort_order,name")->fetchAll();View::render($this->root,'admin/offer-form',['pageTitle'=>'Angebot bearbeiten','offer'=>$o,'version'=>$v->fetch(),'categories'=>$cats]);}
 public function updateOffer(Request $r,array $p):void{
  $id=(int)$p['id'];$this->db->beginTransaction();try{$q=$this->db->prepare('SELECT COALESCE(MAX(version_no),0) FROM offer_versions WHERE offer_id=? FOR UPDATE');$q->execute([$id]);$no=(int)$q->fetchColumn()+1;$v=$this->insertOfferVersion($id,$no,$r);$u=$this->db->prepare('UPDATE offers SET category_id=?,seller_id=?,title=?,status=?,is_private=?,acceptance_deadline=?,current_version_id=?,updated_at=NOW() WHERE id=?');$private=(int)!!$r->input('is_private');$u->execute([(int)$r->input('category_id'),$private?(int)$r->input('seller_id'):null,trim((string)$r->input('title')),(string)$r->input('status'),$private,$r->input('acceptance_deadline')?:null,$v,$id]);$this->db->commit();Session::flash('success','Neue Angebotsversion '.$no.' wurde veröffentlicht/gespeichert.');Response::redirect('/admin/angebote');}catch(\Throwable $e){if($this->db->inTransaction())$this->db->rollBack();throw $e;}
 }
 private function insertOfferVersion(int $offerId,int $no,Request $r):int{
  ...
  $j=fn($k)=>json_encode($r->input($k)?json_decode((string)$r->input($k),true):[],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
  $q->execute([$offerId,$no,trim((string)$r->input('title')),trim((string)$r->input('description')),(float)$r->input('compensation'),(string)$r->input('fulfillment_model'),$r->input('duration_value')!==''?(int)$r->input('duration_value'):null,$r->input('duration_unit')?:null,$j('rules_json'),$j('evidence_json'),$j('start_control_json'),$j('shipping_json'),$j('end_workflow_json'),$j('violation_json'),$j('settings_json')]);return (int)$this->db->lastInsertId();
 }
