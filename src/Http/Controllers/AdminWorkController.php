...
final class AdminWorkController{
 ...
 public function reviewTask(Request $r,array $p):void{$id=(int)$p['executionId'];$decision=(string)$r->input('decision');$q=$this->db->prepare('SELECT te.*,t.order_id,t.order_component_id,t.config_json FROM task_executions te JOIN tasks t ON t.id=te.task_id WHERE te.id=? AND t.order_id=?');$q->execute([$id,(int)$p['id']]);$x=$q->fetch();if(!$x)Response::abort(404);if($decision==='accepted')$this->db->prepare("UPDATE task_executions SET review_status='accepted' WHERE id=?")->execute([$id]);else{$this->db->prepare("UPDATE task_executions SET review_status='rejected' WHERE id=?")->execute([$id]);$cfg=json_decode($x['config_json']?:'{}',true)?:[];$violation=$cfg['violation']??[];if(($violation['missing']??'one_violation')!=='none')$this->ensureViolation((int)$p['id'],'task_not_completed','task_execution',$id,'Zusatzaufgabe wurde als nicht ausreichend bewertet.',$x['order_component_id']?(int)$x['order_component_id']:null);}Session::flash('success','Aufgabe geprüft.');Response::redirect('/admin/auftraege/'.$p['id']);}
