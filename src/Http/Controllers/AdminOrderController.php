...
final class AdminOrderController{
 ...
 public function show(Request $r,array $p):void{
  ...
  $sourceQ=$this->db->prepare("SELECT * FROM order_offer_items WHERE order_id=? ORDER BY sort_order,id");$sourceQ->execute([$o['id']]);$orderOffers=$sourceQ->fetchAll();if(count($orderOffers)>1)$o['title']='Sammelauftrag · '.count($orderOffers).' Angebote';
  ...
  if($orderOffers){
   $availableOptionsQ=$this->db->prepare("SELECT oo.*,ooi.id order_offer_item_id,ooi.title source_offer_title FROM order_offer_items ooi JOIN offer_options oo ON oo.offer_version_id=ooi.offer_version_id WHERE ooi.order_id=? AND oo.is_active=1 AND NOT EXISTS(SELECT 1 FROM order_options x WHERE x.order_id=? AND x.offer_option_id=oo.id AND x.is_active=1) ORDER BY ooi.sort_order,oo.sort_order,oo.id");$availableOptionsQ->execute([$o['id'],$o['id']]);$availableOptions=$availableOptionsQ->fetchAll();
  }else{
   $availableOptionsQ=$this->db->prepare("SELECT oo.*,NULL order_offer_item_id,NULL source_offer_title FROM offer_options oo WHERE oo.offer_version_id=? AND oo.is_active=1 AND NOT EXISTS(SELECT 1 FROM order_options x WHERE x.order_id=? AND x.offer_option_id=oo.id AND x.is_active=1) ORDER BY oo.sort_order,oo.id");$availableOptionsQ->execute([$o['offer_version_id'],$o['id']]);$availableOptions=$availableOptionsQ->fetchAll();
  }
 ...
 public function addOption(Request $r,array $p):void{
  $orderId=(int)$p['id'];$optionId=(int)$r->input('offer_option_id');$this->db->beginTransaction();try{
   ...
   $op=$this->db->prepare("SELECT oo.*,ooi.id order_offer_item_id,ooi.title source_offer_title FROM order_offer_items ooi JOIN offer_options oo ON oo.offer_version_id=ooi.offer_version_id WHERE ooi.order_id=? AND oo.id=? AND oo.is_active=1 LIMIT 1");$op->execute([$orderId,$optionId]);$option=$op->fetch();
   if(!$option){$op=$this->db->prepare("SELECT oo.*,NULL order_offer_item_id,NULL source_offer_title FROM offer_options oo WHERE oo.id=? AND oo.offer_version_id=? AND oo.is_active=1");$op->execute([$optionId,$o['offer_version_id']]);$option=$op->fetch();}
   if(!$option)throw new \RuntimeException('Option ist nicht verfügbar.');
   ...
   $price=(float)$option['price'];$this->db->prepare('INSERT INTO order_options(order_id,order_offer_item_id,offer_option_id,name,price,config_snapshot,created_at) VALUES(?,?,?,?,?,?,NOW())')->execute([$orderId,$option['order_offer_item_id']?:null,$optionId,$option['name'],$price,$option['requirements_json']]);$orderOptionId=(int)$this->db->lastInsertId();
