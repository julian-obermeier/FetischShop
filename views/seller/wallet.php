...
<h2>Auszahlungsanträge</h2><div class="worklist"><?php foreach($requests??[] as $p):?><div class="work-row"><div><b><?=number_format((float)$p['net_amount'],2,',','.')?> € netto</b><span><?=App\Core\View::e($p['status'])?> · <?=date('d.m.Y H:i',strtotime($p['requested_at']))?></span></div><?php if($p['status']==='requested'):?><form method="post" action="/konto/wallet/auszahlung/<?=$p['id']?>/zurueckziehen"><?=App\Core\Csrf::field()?><button class="btn ghost">Zurückziehen</button></form><?php endif;?></div><?php endforeach;?><?php if(empty($requests)):?><div class="empty">Noch keine Auszahlungsanträge.</div><?php endif;?></div>
