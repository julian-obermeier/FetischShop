...
?>
<main class="section">
<div class="admin-page-intro"><div><span class="eyebrow">Auftragsverwaltung</span><h1>Aufträge</h1><p>Alle laufenden und abgeschlossenen Aufträge zentral durchsuchen und filtern.</p></div></div>
<form method="get" class="card filters">
<input type="search" name="q" value="<?=App\Core\View::e($filterTerm)?>" placeholder="Auftragsnummer, Name, E-Mail oder Angebot">
<select name="status"><option value="">Alle Status</option><?php foreach($statuses as $s):?><option value="<?=App\Core\View::e($s)?>" <?=$filterStatus===$s?'selected':''?>><?=App\Core\View::e($statusLabels[$s]??$s)?></option><?php endforeach;?></select>
<select name="phase"><option value="">Alle Phasen</option><?php foreach($phases as $p):?><option value="<?=App\Core\View::e($p)?>" <?=$filterPhase===$p?'selected':''?>><?=App\Core\View::e($phaseLabels[$p]??$p)?></option><?php endforeach;?></select>
...
</tbody></table><?php if(!$orders):?><div class="empty">Keine Aufträge passend zu den Filtern gefunden.</div><?php endif;?></div>
</main>