...
?>
<main class="section">
<div class="seller-hero"><div><span class="eyebrow">Finanzen</span><h1>Wallet & Auszahlungen</h1><p>Hier siehst du, was vorgemerkt, in Prüfung oder bereits frei verfügbar ist.</p></div></div>

<div class="seller-wallet-grid">
<div class="seller-wallet-box"><span>Vorgemerkt</span><strong><?=number_format((float)($wallet['balance_reserved']??0),2,',','.')?> €</strong><small class="muted">aus laufenden Aufträgen</small></div>
<div class="seller-wallet-box"><span>In Prüfung</span><strong><?=number_format((float)($wallet['balance_in_review']??0),2,',','.')?> €</strong><small class="muted">Abschlussprüfung läuft</small></div>
<div class="seller-wallet-box available"><span>Verfügbar</span><strong><?=number_format((float)($wallet['balance_available']??0),2,',','.')?> €</strong><small class="muted">kann ausgezahlt werden</small></div>
</div>

<div class="seller-home-grid">
<section class="card admin-form"><h2>Auszahlungsmethode</h2><p class="muted">Hinterlege mindestens eine Methode. Neue Angaben ersetzen die bisher aktive Methode desselben Typs.</p>
...
<?php endif;?>
<?php if($methods):?><h3>Aktive Auszahlungsmethoden</h3><div class="seller-action-list"><?php foreach($methods as $m): if(!$m['is_active'])continue;?><div class="seller-action"><div><b><?=App\Core\View::e($m['method_type']==='bank'?'Banküberweisung':'PayPal')?></b><span><?php if($m['method_type']==='bank'):?><?=App\Core\View::e($m['account_holder'])?> · <?=App\Core\View::e($m['iban'])?><?php else:?><?=App\Core\View::e($m['paypal_identifier'])?><?php endif;?></span></div><span class="seller-status ok">Aktiv</span></div><?php endforeach;?></div><?php endif;?>
</section>

<section class="card admin-form"><h2>Auszahlung beantragen</h2><p class="muted">Mindestbetrag: <?=number_format((float)($payoutSettings['payout_minimum']??10),2,',','.')?> € · Bearbeitungstage: <?=App\Core\View::e($payoutSettings['payout_days']??'nach Einstellung')?></p>
...
<section class="seller-section-card"><div class="section-head compact"><div><h2>Auszahlungsverlauf</h2><p class="muted">Status und zugeordnete Aufträge deiner Auszahlungen.</p></div></div><div class="seller-action-list">
