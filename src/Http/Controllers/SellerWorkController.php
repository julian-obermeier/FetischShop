...
final class SellerWorkController{
 ...
 public function spontaneous(Request $r,array $p):void{
  $o=$this->order((int)$p['id']);try{$rid=(int)$r->input('request_id');$q=$this->db->prepare('SELECT * FROM spontaneous_requests WHERE id=? AND order_id=?');$q->execute([$rid,$o['id']]);$req=$q->fetch();if(!$req)throw new RuntimeException('Anforderung nicht gefunden.');if(time()>strtotime($req['grace_ends_at']))throw new RuntimeException('Die Nachfrist ist abgelaufen.');$f=$this->storage()->storeUploaded($r->files['evidence']??[],'evidence',['image/jpeg','image/png','image/webp'],12*1024*1024,480);$run=$this->db->prepare('SELECT id FROM order_runs WHERE order_id=? ORDER BY run_no DESC LIMIT 1');$run->execute([$o['id']]);$captureSource=in_array((string)$r->input('capture_source'),['live_camera','file_picker'],true)?(string)$r->input('capture_source'):'file_picker';$meta=json_encode(['capture_source'=>$captureSource,'user_agent'=>$r->server['HTTP_USER_AGENT']??null,'image_width'=>$f['image_width']??null,'image_height'=>$f['image_height']??null,'image_megapixels'=>$f['image_megapixels']??null],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$this->db->prepare("INSERT INTO evidences(order_id,order_run_id,order_component_id,spontaneous_request_id,evidence_type,file_path,original_name,mime_type,file_size,sha256,captured_at,metadata_json,review_status,created_at) VALUES(?,?,?,?,'spontaneous',?,?,?,?,?,NOW(),?,'pending',NOW())")->execute([$o['id'],$run->fetchColumn(),$req['order_component_id']?:null,$rid,$f['path'],$f['original_name'],$f['mime'],$f['size'],$f['sha256'],$meta]);$c=$this->db->prepare('SELECT COUNT(*) FROM evidences WHERE spontaneous_request_id=?');$c->execute([$rid]);if((int)$c->fetchColumn()>=(int)$req['requested_count'])$this->db->prepare("UPDATE spontaneous_requests SET status='uploaded' WHERE id=?")->execute([$rid]);Session::flash('success','Zusatzfoto wurde eingereicht.');}catch(\Throwable $e){Session::flash('error',$e->getMessage());}Response::redirect('/konto/auftraege/'.$o['id']);
 }
 public function submitTask(Request $r,array $p):void{
  ...
  try{
   ...
   foreach($photos as $photoIndex=>$photo){
    ...
    for($n=0;$n<$count;$n++){
     $fileKey='task_photo_'.$photoIndex.'_'.$n;
     $file=$this->storage()->storeUploaded($r->files[$fileKey],'evidence',['image/jpeg','image/png','image/webp'],12*1024*1024,480);
     $captureSource=in_array((string)$r->input('capture_source'),['live_camera','file_picker'],true)?(string)$r->input('capture_source'):'file_picker';$meta=json_encode(['label'=>(string)($photo['label']??'Pflichtfoto'),'position'=>$n+1,'capture_source'=>$captureSource,'user_agent'=>$r->server['HTTP_USER_AGENT']??null,'image_width'=>$file['image_width']??null,'image_height'=>$file['image_height']??null,'image_megapixels'=>$file['image_megapixels']??null],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);
 ...
 public function reportDamage(Request $r,array $p):void{
  ...
  try{
   ...
   $file=$this->storage()->storeUploaded($r->files['evidence']??[],'evidence',['image/jpeg','image/png','image/webp'],12*1024*1024,480);
   $run=$this->db->prepare('SELECT id FROM order_runs WHERE order_id=? ORDER BY run_no DESC LIMIT 1');$run->execute([$o['id']]);$runId=(int)$run->fetchColumn();
   $this->db->beginTransaction();
   $captureSource=in_array((string)$r->input('capture_source'),['live_camera','file_picker'],true)?(string)$r->input('capture_source'):'file_picker';$meta=json_encode(['capture_source'=>$captureSource,'user_agent'=>$r->server['HTTP_USER_AGENT']??null,'image_width'=>$file['image_width']??null,'image_height'=>$file['image_height']??null,'image_megapixels'=>$file['image_megapixels']??null],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$this->db->prepare("INSERT INTO evidences(order_id,order_run_id,order_component_id,evidence_type,file_path,original_name,mime_type,file_size,sha256,captured_at,metadata_json,review_status,created_at) VALUES(?,?,?,'damage_initial',?,?,?,?,?,NOW(),?,'pending',NOW())")->execute([$o['id'],$runId,$componentId,$file['path'],$file['original_name'],$file['mime'],$file['size'],$file['sha256'],$meta]);
 ...
 public function damageResponse(Request $r,array $p):void{
  ...
  $payload=[];if(in_array($req['field_type'],['photo','video'],true)){try{$allowed=$req['field_type']==='video'?['video/mp4','video/webm']:['image/jpeg','image/png','image/webp'];$minSide=$req['field_type']==='photo'?480:0;$f=$this->storage()->storeUploaded($r->files['evidence']??[],'evidence',$allowed,80*1024*1024,$minSide);$run=$this->db->prepare('SELECT order_run_id,order_component_id FROM damage_cases WHERE id=?');$run->execute([$req['damage_case_id']]);$case=$run->fetch();$captureSource=in_array((string)$r->input('capture_source'),['live_camera','file_picker'],true)?(string)$r->input('capture_source'):'file_picker';$meta=json_encode(['capture_source'=>$captureSource,'user_agent'=>$r->server['HTTP_USER_AGENT']??null,'image_width'=>$f['image_width']??null,'image_height'=>$f['image_height']??null,'image_megapixels'=>$f['image_megapixels']??null],JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);$this->db->prepare("INSERT INTO evidences(order_id,order_run_id,order_component_id,damage_evidence_request_id,evidence_type,file_path,original_name,mime_type,file_size,sha256,captured_at,metadata_json,review_status,created_at) VALUES(?,?,?,?,'damage_followup',?,?,?,?,?,NOW(),?,'pending',NOW())")->execute([$o['id'],$case['order_run_id'],$case['order_component_id'],$reqId,$f['path'],$f['original_name'],$f['mime'],$f['size'],$f['sha256'],$meta]);$payload=['evidence_id'=>(int)$this->db->lastInsertId()];}catch(\Throwable $e){Session::flash('error',$e->getMessage());Response::redirect('/konto/auftraege/'.$o['id']);}}else{$payload=['text'=>trim((string)$r->input('response'))];}
