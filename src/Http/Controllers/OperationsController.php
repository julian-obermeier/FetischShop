...
final class OperationsController
{
    ...
    public function createOutage(Request $r): void
    {
        try {
            ...
            $this->db->prepare("UPDATE evidences SET retake_deadline=DATE_ADD(retake_deadline,INTERVAL ? MINUTE),retake_grace_ends_at=DATE_ADD(retake_grace_ends_at,INTERVAL ? MINUTE) WHERE review_status='rejected' AND resolved_by_evidence_id IS NULL AND retake_grace_ends_at IS NOT NULL AND retake_grace_ends_at>=?")->execute([$minutes,$minutes,$startSql]);
            ...
            $this->db->prepare("UPDATE digital_components SET deadline=DATE_ADD(deadline,INTERVAL ? MINUTE),updated_at=NOW() WHERE deadline IS NOT NULL AND deadline>=? AND status IN('open','draft','running')")->execute([$minutes,$startSql]);
            ...
            $this->db->prepare("UPDATE shipping_steps ss JOIN shipping_workflows sw ON sw.id=ss.shipping_workflow_id SET ss.deadline=DATE_ADD(ss.deadline,INTERVAL ? MINUTE) WHERE ss.deadline IS NOT NULL AND ss.deadline>=? AND ss.status='pending' AND sw.status='active'")->execute([$minutes,$startSql]);
