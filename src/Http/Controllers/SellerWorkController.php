...
final class SellerWorkController{
 ...
 public function spontaneous(Request $r,array $p):void{
  $o=$this->order((int)$p['id']);try{$rid=(int)$r->input('request_id');$q=$this->db->prepare('SELECT * FROM spontaneous_requests WHERE id=? AND order_id=?');$q->execute([$rid,$o['id']]);$req=$q->fetch();if(!$req)throw new RuntimeException('Anforderung nicht gefunden.');if(time()>strtotime($req['grace_ends_at']))throw new RuntimeException('Die Nachfrist ist abgelaufen.');$f=$this->storage()->storeUploaded($r->files['evidence']??[],'evidence',['image/jpeg','image/png','image/webp'],12*1024*1024);$run=$this->db->prepare('SELECT id FROM order_runs WHERE order_id=? ORDER BY run_no DESC LIMIT 1');$run->execute([$o['id']]);$this->db->prepare("INSERT INTO evidences(order_id,order_run_id,spontaneous_request_id,evidence_type,file_path,original_name,mime_type,file_size,sha256,captured_at,metadata_json,review_status,created_at) VALUES(?,?,?,'spontaneous',?,?,?,?,?,NOW(),'{}','pending',NOW())")->execute([$o['id'],$run->fetchColumn(),$rid,$f['path'],$f['original_name'],$f['mime'],$f['size'],$f['sha256']]);$c=$this->db->prepare('SELECT COUNT(*) FROM evidences WHERE spontaneous_request_id=?');$c->execute([$rid]);if((int)$c->fetchColumn()>=(int)$req['requested_count'])$this->db->prepare("UPDATE spontaneous_requests SET status='uploaded' WHERE id=?")->execute([$rid]);Session::flash('success','Zusatzfoto wurde eingereicht.');}catch(\Throwable $e){Session::flash('error',$e->getMessage());}Response::redirect('/konto/auftraege/'.$o['id']);
 }
