...
final class InstallerController{
 ...
 public function show():void{$checks=['PHP >= 8.1'=>version_compare(PHP_VERSION,'8.1.0','>='),'PDO MySQL'=>extension_loaded('pdo_mysql'),'Fileinfo'=>extension_loaded('fileinfo'),'mbstring'=>extension_loaded('mbstring'),'config/ beschreibbar'=>is_writable($this->root.'/config'),'storage/ beschreibbar'=>is_dir($this->root.'/storage')?is_writable($this->root.'/storage'):is_writable($this->root)];View::render($this->root,'install',['checks'=>$checks,'error'=>null]);}
