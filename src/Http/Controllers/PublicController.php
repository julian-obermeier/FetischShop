...
final class PublicController{
 ...
 public function offer(Request $r,array $p):void{
  $s=$this->db->prepare("SELECT o.id offer_id,o.current_version_id,o.title offer_public_title,o.is_private,o.seller_id private_seller_id,o.acceptance_deadline,c.name category_name,c.is_digital,ov.* FROM offers o JOIN categories c ON c.id=o.category_id JOIN offer_versions ov ON ov.id=o.current_version_id WHERE o.id=? AND o.status='active' AND o.is_private=0");$s->execute([(int)$p['id']]);$offer=$s->fetch();if(!$offer)Response::abort(404,'Angebot nicht gefunden.');
  $opt=$this->db->prepare("SELECT * FROM offer_options WHERE offer_version_id=? AND is_active=1 ORDER BY sort_order,id");$opt->execute([$offer['current_version_id']]);$seller=$this->auth->seller();
  View::render($this->root,'public/offer',['pageTitle'=>$offer['title'],'offer'=>$offer,'options'=>$opt->fetchAll(),'seller'=>$seller]);
 }
