<main class="section">
<div class="section-head"><div><span class="eyebrow">Versand</span><h1>Versandadressen</h1></div><a class="btn ghost" href="/admin">Zurück</a></div>
<section class="card admin-form"><h2>Neue Versandadresse</h2><p class="muted">Diese Adressen können später einem Versandworkflow zugewiesen werden.</p><form method="post" action="/admin/empfaengeradressen" class="form-grid"><?=App\Core\Csrf::field()?>
...
<section><h2>Gespeicherte Versandadressen</h2><div class="worklist"><?php foreach($addresses as $a):?><div class="card"><form method="post" action="/admin/empfaengeradressen" class="form-grid"><?=App\Core\Csrf::field()?><input type="hidden" name="id" value="<?=$a['id']?>">
...
<button class="btn full">Änderungen speichern</button></form></div><?php endforeach;?><?php if(!$addresses):?><div class="empty">Es wurde noch keine Versandadresse hinterlegt.</div><?php endif;?></div></section>
</main>